...
class GameManager:
    ...
    @property
    def current_traders(self) -> list[Trader]:
        return self.traders[self.current_map_key]
        ...
        if self.should_change_scene:
            ...
            if self.player:
                ...
                if self.player.direction == Direction.LEFT:
                    dx = -GameSettings.TILE_SIZE
                elif self.player.direction == Direction.RIGHT:
                    dx = GameSettings.TILE_SIZE
                elif self.player.direction == Direction.UP:
                    dy = -GameSettings.TILE_SIZE
                elif self.player.direction == Direction.DOWN:
                    dy = GameSettings.TILE_SIZE
                self.player.position = Position(
                    self.maps[self.current_map_key].spawn.x + dx,
                    self.maps[self.current_map_key].spawn.y + dy
                ) 
    ...
    def check_collision(self, rect: pg.Rect) -> bool:
        ...
        for entity in self.enemy_trainers[self.current_map_key]:
            if rect.colliderect(entity.animation.rect):
                return True
        for entity in self.traders[self.current_map_key]:
            if rect.colliderect(entity.animation.rect):
                return True
        
        return False
    def check_bush_collision(self,rect:pg.Rect) -> bool:
        if self.maps[self.current_map_key].check_collision_bush(rect):
            return True
        return False
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        for key, m in self.maps.items():
            ...
            block["traders"] = [t.to_dict() for t in self.traders.get(key, [])]
    ...
    def from_dict(cls, data: dict[str, object]) -> "GameManager":
        ...
        Logger.info("Loading traders")
        for m in data["map"]:
            raw_data = m["traders"]
            gm.traders[m["path"]] = [Trader.from_dict(t, gm) for t in raw_data]
